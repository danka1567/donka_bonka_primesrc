...
async def _resolve_one_flaresolverr(
    ...
    async with sem:
        ...
        for attempt in range(reloads + 1):
            ...
            # Use a unique session per URL to prevent caching/state reuse issues
            unique_session_id = f"{session_id}_{index}_{attempt}"
            
            try:
                # Create a fresh session for this specific request
                await loop.run_in_executor(
                    None,
                    lambda: _fs_post(base_url, {
                        "cmd": "sessions.create",
                ...
                # Destroy the unique session after this request to prevent state reuse
                try:
                    await loop.run_in_executor(
                        None,
                        lambda: _fs_post(base_url, {
                    ...
                    pass  # Session cleanup failure is not critical
...
async def stage2_extract_stream_urls(
    ...
    # Note: Each URL will use its own unique session to prevent caching/state reuse
    log_info("Using unique sessions per URL to prevent response caching")
